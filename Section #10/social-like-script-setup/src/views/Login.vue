...
const onSubmit = () => {
    const cryptedPassword = CryptoJS.HmacSHA1(userData.value.password, store.getters._saltKey).toString();
    appAxios.get(`/users?username=${userData.value.username}&password=${cryptedPassword}`).then(login_response => {
